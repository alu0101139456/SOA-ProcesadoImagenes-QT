...
void GaussianBlur::ApplyGaussianFilterToImage( eImagen& output)
{
      QRgb **temp = new QRgb* [size_];
    for (int i = 0; i < size_; i++)
        temp[i] = new QRgb [size_];

    for (int i = 0; i < output.imagen.width(); i++)
        for (int j = 0; j < output.imagen.height(); j++)
        {
            GetPixelMatrix(QPoint(i, j), output.imagen, temp);
            output.imagen.setPixel(QPoint(i,j), GetNewPixelValue(temp));
        }
